...
unpaired <- unpaired %>% 
  ...
  mutate(bedLocation = ifelse(
    unpaired$seq_libtype == "one", "/fh/fast/paguirigan_a/pub/ReferenceDataSets/reagent_specific_data/sequencing_panel_bed/TruSight-One-Illumina/TruSight_One_v1.1-liftover-hg38.bed",
    ifelse(unpaired$seq_libtype == "tsm", "/fh/fast/paguirigan_a/pub/ReferenceDataSets/reagent_specific_data/sequencing_panel_bed/TruSight-Myeloid-Illumina/trusight-myeloid-amplicon-v4-track_interval-liftOver-hg38.bed",
           ifelse(unpaired$seq_libtype == "washu", "/fh/fast/paguirigan_a/pub/ReferenceDataSets/reagent_specific_data/sequencing_panel_bed/WASHU-AML/washU_intervals-remapped-hg38.bed",
                  ifelse(unpaired$seq_libtype == "agssex6", "/fh/fast/paguirigan_a/pub/ReferenceDataSets/reagent_specific_data/sequencing_panel_bed/AgilentSureSelectv6-Exome/S07604514_Covered-remapped-hg38.bed",
                         ifelse(unpaired$seq_libtype == "agssex5", "/fh/fast/paguirigan_a/pub/ReferenceDataSets/reagent_specific_data/sequencing_panel_bed/AgilentSureSelectv5-Exome/S04380110_Covered-remapped-hg38.bed",
                                ifelse(unpaired$seq_libtype == "tapestriAMLPanel", "/fh/fast/paguirigan_a/pub/ReferenceDataSets/reagent_specific_data/sequencing_panel_bed/MissionBio-AMLPanel/2019-02-13-MissionBioAML.bed", "")))))))
...
# Analysis Workflow
## Choose a Workflow
Workflow is:  tg-wdl-unpairedVariantCaller
Download the current master branch copy and unzip the file. 
```{r}
workflowDir <- "./Workflows/testingTrio/"
...
unzip(zipfile = paste0(workflowDir, "master.zip"),
      exdir = workflowDir)
file.remove(paste0(workflowDir, "master.zip"))

list.dirs(path = workflowDir)
workflowFiles <- "./Workflows/testingTrio//tg-wdl-unpairedVariantCaller-master/gizmo/"
```

## Make a Manifest

```{r}
manifest <- preselect %>% select(omics_sample_name, bamLocation, bedLocation, molecular_id) %>% rename("sampleName" = "omics_sample_name")
...
```{r}
batchFileName <- paste0("cromwell-manifests/", repoName, "/", format(Sys.Date(), "%Y-%m-%d-"), "HALOValidation-trio.tsv")
...
thisManifest <- "cromwell-manifests/FredHutch/tg-wdl-unpairedVariantCaller/2020-03-10-HALOValidation-trio.tsv"
```
...
womtoolValidate(paste0(workflowFiles, "unpaired-trio-variantcaller-gizmo.wdl"))

thisJob <- cromwellSubmitBatch(
  WDL = paste0(workflowFiles, "unpaired-trio-variantcaller-gizmo.wdl"),
  Batch = paste0(workflowFiles, "unpaired-trio-variantcaller-gizmo-batch.json"),
  Params = paste0(workflowFiles, "unpaired-trio-variantcaller-gizmo-parameters.json"),
  Options = paste0(workflowDir, "gizmo-options.json"),
  Labels = data.frame("workflowType" = "triotesting"))
...
Jobs of interest
```{r}
thisOne <- "138eb321-035a-4ad2-8fcd-c3d2c0bc2748"
jobs <- cromwellJobs()
```
